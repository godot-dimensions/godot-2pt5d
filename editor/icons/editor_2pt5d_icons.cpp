#include "editor_2pt5d_icons.h"

#include "editor_2pt5d_icons.gen.h"
...
Ref<ImageTexture> _generate_editor_2pt5d_icon(const String &p_icon_name) {
	...
	while (source_index < editor_2pt5d_icons_count) {
		if (String(editor_2pt5d_icons_names[source_index]) == p_icon_name) {
			break;
		}
		source_index++;
	}
	if (source_index == editor_2pt5d_icons_count) {
		WARN_PRINT("Godot 2.5D: Icon not found: '" + p_icon_name + "'.");
	...
	Error err = ImageLoaderSVG::create_image_from_string(img, editor_2pt5d_icons_sources[source_index], scale, false, color_map);
#else
	...
	Error err = img->load_svg_from_string(editor_2pt5d_icons_sources[source_index], scale);
#endif
	ERR_FAIL_COND_V_MSG(err != OK, Ref<ImageTexture>(), "Godot 2.5D: Failed generating SVG icon.");
